...
from hecos.app.application import HecosApplication
...
def main():
    """Starts the Hecos application."""
    app = HecosApplication()
    try:
        app.run()
    finally:
...
if __name__ == "__main__":
    if not os.environ.get("HECOS_MONITORED_PROCESS"):
        if not instance_lock.acquire_lock("hecos_console"):
            print("\n[ERROR] Another instance of Hecos Console is already running.")
            sys.exit(1)
