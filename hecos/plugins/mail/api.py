...
def _get_cfg() -> dict:
    ...
    try:
        ...
        return cfg.get("plugins", {}).get("MAIL", {})
    except Exception:
        return {}
...
def update_app_password():
    ...
    try:
        from hecos.plugins.mail.credential_helper import set_mail_credentials
...
def get_account_info():
    ...
    try:
        from hecos.plugins.mail.credential_helper import get_user_email, get_user_app_password
...
def update_mail_config():
    """Persists MAIL plugin settings to plugins.yaml via ConfigManager."""
    data = request.get_json(force=True) or {}
    ALLOWED = {
    ...
    try:
        from hecos.app.config import ConfigManager
        cm = ConfigManager()
        current = dict(cm.get_plugin_config("MAIL") or {})
        for key, val in data.items():
            if key in ALLOWED:
                if key in ("smtp_port", "imap_port", "max_messages"):
                    ...
                    val = bool(val)
                current[key] = val
        if "plugins" not in cm.config:
            cm.config["plugins"] = {}
        cm.config["plugins"]["MAIL"] = current
        ok = cm.save()
