...
# Scrolling TUI Handler (for hecos.ui.interface)
class ScrollingTUIHandler(logging.Handler):
    """Log handler that uses Hecos's TUI scrolling region to avoid prompt clobbering."""
...
def init_logger(config, allow_external_windows=True):
    ...
    if destination_lower == 'console':
        ...
        if message_types == 'info' or message_types == 'both':
            ps_script_info = (
                "$host.ui.RawUI.WindowTitle = 'Hecos - Activity Log'; "
...
def open_debug_log():
    ...
    ps_script_debug = (
        "$host.ui.RawUI.WindowTitle = 'Hecos - Technical Debug (LiteLLM)'; "
...
def close_debug_log():
    ...
    try:
        subprocess.run('taskkill /FI "WINDOWTITLE eq Hecos - Technical Debug (LiteLLM)*" /F', 
...
def close_activity_log():
    ...
    try:
        ...
        subprocess.run('taskkill /FI "WINDOWTITLE eq Hecos - Activity Log*" /F', shell=True, capture_output=True)
        subprocess.run('taskkill /FI "WINDOWTITLE eq Hecos - Log Attivit\u00e0*" /F', shell=True, capture_output=True)
    except:
        pass
