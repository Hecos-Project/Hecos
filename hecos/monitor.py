"""
MODULE: Simplified Resuscitation Monitor - Hecos
...
try:
    from hecos.core.logging.hub import get_hub
except ImportError:
    get_hub = None
...
# Path configuration
DEFAULT_MAIN_SCRIPT = os.path.join("hecos", "main.py")
CONFIG_FILE = os.path.join("hecos", "config", "data", "system.yaml")

# Logging configuration
LOGS_DIR = os.path.join("hecos", "logs")
os.makedirs(LOGS_DIR, exist_ok=True)
MONITOR_LOG = os.path.join(LOGS_DIR, "hecos_monitor.log")
...
def get_translator():
    ...
    translations = {
        "it": {
            ...
            "starting": "Starting Hecos ({script})...",
...
def start_and_monitor(script_to_run):
    # For module-style runs (like hecos.modules.web_ui.server), we don't check file existence directly if it contains dots
    ...
    last_code_time = get_max_py_mtime(hecos_folder)
    ...
    try:
        ...
        while process.poll() is None:
            ...
            if iterations % 5 == 0:
                current_code_time = get_max_py_mtime(hecos_folder)
                if current_code_time > last_code_time + 2:
                    monitor_log(f"Code change detected in hecos/ folder. Restarting...")
                    process.terminate()
                    process.wait(timeout=5)
                    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hecos Watchdog Monitor")
    ...
    print(f"\n{'-'*55}")
    print(f" [MONITOR] Hecos Watchdog Active")
    monitor_log(f"Target: {args.script}")
    print(f"{'-'*55}\n")
    
    # Determine lock name based on script
    lock_name = "hecos_console" if "main.py" in args.script else "hecos_web"
    
    if not instance_lock.acquire_lock(lock_name):
        print(f"\n[MONITOR] ERROR: Another instance of Hecos ({lock_name}) is already running.")
        ...
        while True:
            try:
                ...
                if not should_restart:
                    print(f"\n[MONITOR] Hecos shut down normally. Exiting watchdog.")
                    break
                
                # If we are here, we need to restart
                print(f"\n[MONITOR] Restarting Hecos in progress...")
