"""
Chat API routes for the native Hecos Web UI.
Provides:
...
import time
import logging
from hecos.core.constants import LOGS_DIR, SNAPSHOTS_DIR, IMAGES_DIR, AUDIO_DIR
...
def generate_voice_file(text: str, voice_cfg: dict) -> str:
    ...
    """
    try:
        # Dynamically resolve Hecos root directory (from web_ui/routes_chat.py going up to root)
        ...
        # risposta.wav in hecos/media/audio/
    ...
    def api_audio():
        ...
        if _last_audio_path and os.path.exists(_last_audio_path):
            return send_file(_last_audio_path, mimetype="audio/wav",
                             download_name="hecos_response.wav")
