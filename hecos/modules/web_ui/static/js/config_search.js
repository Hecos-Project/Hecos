/**
 * Hecos Config Search Engine v0.2.0
...
(function () {
    ...
    // ── Settings ──────────────────────────────────────────────────────────────
    const SETTINGS_KEY = 'hecos-search-settings';
    const DEFAULTS = {
    ...
    // ── Injection ─────────────────────────────────────────────────────────────
    function inject() {
        if (document.getElementById('hecos-search-bar')) return;
        const anchor = document.getElementById('config-filter-tabs');
        if (!anchor) return;

        // --- SEARCH BAR ---
        const bar = document.createElement('div');
        bar.id = 'hecos-search-bar';
        bar.innerHTML = `
    ...
    function buildSettingsHTML() {
        ...
        <div style="display:flex;gap:8px;margin-top:14px;">
            <button onclick="window.HecosSearch.save()" style="flex:1;padding:6px;background:var(--accent,#66fcf1);color:#0b0c10;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;">Save</button>
            <button onclick="window.HecosSearch.reset()" style="flex:1;padding:6px;background:rgba(255,50,50,.15);color:#ff5555;border:1px solid rgba(255,50,50,.3);border-radius:8px;font-size:12px;cursor:pointer;">Reset</button>
        </div>
