...
window.hpmToggleSelectAll = function () {
    ...
    const targetPkgs = (window.HPM_STATE && window.HPM_STATE.filteredPackages)
        ? window.HPM_STATE.filteredPackages
        : (window._packages || []);
...
window.hpmUpdateSelectionUI = function () {
    ...
    // Sync select-all checkbox state based on currently visible packages
    const chkAll = document.getElementById('hpm-select-all');
    if (chkAll && window.HPM_STATE && window.HPM_STATE.filteredPackages) {
        const visibleRemovable = window.HPM_STATE.filteredPackages.filter(p => p.removable === true);
        if (visibleRemovable.length > 0) {
            chkAll.checked = visibleRemovable.every(p => window._hpmSelectedPackages.has(p.id));
        } else {
