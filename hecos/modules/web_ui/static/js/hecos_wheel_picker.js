    ...
    function build() {
      ...
      Object.entries(tracks).forEach(([key, trk]) => {
        ...
        trk.parentElement.addEventListener('mousedown', () => {
          const targetColIdx = colKeys.indexOf(key);
          wasActive = (activeColIdx === targetColIdx);
          _setActiveCol(targetColIdx);
        });

        trk.addEventListener('click', (e) => {
          if (!wasActive) return;
          const itemEl = e.target.closest('.hwp-item');
          if (!itemEl) return;
          const items = Array.from(trk.querySelectorAll('.hwp-item'));
          const idx = items.indexOf(itemEl);
          if (idx > -1) {
            trk.scrollTo({ top: idx * ITEM_H, behavior: 'smooth' });
          }
        });
