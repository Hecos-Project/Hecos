...
title HECOS -- Native Web Server (Watchdog)
color 0B
...
echo  ==============================================================
echo   HECOS NATIVE WEB INTERFACE v%HECOS_VERSION%
echo  ==============================================================
echo.
...
netsh advfirewall firewall show rule name="Hecos WebUI LAN" >nul 2>&1
if errorlevel 1 (
  echo [*] Opening port 7070 in Windows Firewall...
  netsh advfirewall firewall add rule name="Hecos WebUI LAN" dir=in action=allow protocol=TCP localport=7070 >nul 2>&1
  echo [+] Port opened.
) else (
...
set SCHEME=http
for /f "delims=" %%s in ('python -c "import yaml; print('https' if yaml.safe_load(open('hecos/config/data/system.yaml')).get('plugins',{}).get('WEB_UI',{}).get('https_enabled',False) else 'http')" 2^>nul') do set SCHEME=%%s
