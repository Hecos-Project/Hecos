# 🛡️ 19. Sicurezza Avanzata (Hecos PKI)

Hecos mette la sicurezza al centro, specialmente quando si accede all'interfaccia da altri dispositivi.

- **Hecos CA**: Il sistema funge da propria Autorità di Certificazione (Root CA), generando certificati HTTPS sicuri e unici per la tua macchina.
...
- **Privacy a 3 Livelli (v0.26.0)**: La WebUI offre controlli granulari sulla persistenza dei dati:
  - `Normale`: Persistenza completa su disco.
  - `Auto-Wipe`: Messaggi solo in RAM, cancellati al riavvio del sistema.
  - `Incognito`: Traccia zero. Hecos opera esclusivamente in RAM e il contesto viene rimosso al cambio della chat, garantendo che nessuna traccia della conversazione rimanga sul server fisico.
