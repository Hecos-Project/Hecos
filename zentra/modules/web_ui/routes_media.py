...
def init_media_routes(app, cfg_mgr, root_dir, logger, get_sm=None):
    ...
    @app.route("/zentra/api/media/presets", methods=["GET"])
    def list_media_presets():
        """Returns all preset names (built-in + user) with metadata."""
        try:
            from zentra.plugins.image_gen.presets import BUILTIN_PRESETS
    ...
    @app.route("/zentra/api/media/presets/load/<path:name>", methods=["GET"])
    def load_media_preset(name):
        """Returns the full config dict for a named preset."""
        try:
            from zentra.plugins.image_gen.presets import get_preset
            from zentra.core.media_config import get_media_config
    ...
    @app.route("/zentra/api/media/presets/save", methods=["POST"])
    def save_media_preset():
        ...
        try:
            ...
            from zentra.plugins.image_gen.presets import save_user_preset
    ...
    @app.route("/zentra/api/media/presets/delete/<path:name>", methods=["DELETE"])
    def delete_media_preset(name):
        """Deletes a user preset by name. Built-ins cannot be deleted."""
        try:
            from zentra.plugins.image_gen.presets import delete_user_preset
    ...
    @app.route("/zentra/api/media/hf-search", methods=["GET"])
    def search_huggingface_hub():
        ...
        try:
            ...
            headers = {"User-Agent": "ZentraCore/0.18.2"}
            from zentra.core.media.image_providers.utils import get_proxies
